...
public class CommentPanel extends ContentWidget {
	...
	private void setupReplyLayout() {
    	...
    	replyPanel.setHeading(Resources.messages.Comment_reply((String)reply.get(IComment.EMAIL),(String)reply.get(IComment.USER_NAME),new Timestamp((Long)reply.get(IComment.ADD_TIME)).toString()));
        ...
        ipAddress.setText(Resources.constants.CommentList_ipAddress()+":"+reply.get(IComment.IP_ADDRESS));
	...
	private void setupCommentLayout() {
		...
		if( comment.getString( IComment.USER_ID ) == null ){
			comment.set(IComment.USER_NAME, Resources.constants.CommentList_anonymous());
		}
		...
		commentPanel.setHeading(Resources.messages.Comment_to((String)comment.get(IComment.EMAIL),(String)comment.get(IComment.USER_NAME),new Timestamp((Long)comment.get(IComment.ADD_TIME)).toString(),(String)comment.get(IComment.ID_VALUE)));
        ...
        td.setHorizontalAlign(HorizontalAlignment.RIGHT);
        Label ipAddress = new Label();
        ipAddress.setText(Resources.constants.CommentList_rank()+":"+comment.getString(IComment.COMMENT_RANK)+"  "+Resources.constants.CommentList_ipAddress()+":"+comment.getString(IComment.IP_ADDRESS));
	    ...
	    public void componentSelected(ButtonEvent sender) {
	    	try {
	    		if(comment.get(IComment.STATUS).equals(IComment.STATUS_ACTIVE)){
	    			...
	    			new UpdateService().updateBean(comment.getString(IComment.PK_ID), comment, new UpdateService.Listener(){
	...
	private void submit() {
		Map<String, Object> props = FormUtils.getPropsFromForm(formPanel);
		BeanObject form = new BeanObject(ModelNames.COMMENT, props);
		if(reply.get(IComment.PK_ID) != null ){
			reply.set(IComment.CONTENT,form.get(IComment.CONTENT));
			reply.set(IComment.ADD_TIME, System.currentTimeMillis());
			reply.set(IComment.STATUS, IComment.STATUS_ACTIVE);
			new UpdateService().updateBean((String)reply.get(IComment.PK_ID), reply, new UpdateService.Listener(){
	    	...
	    	form.set(IComment.ADD_TIME, System.currentTimeMillis());
	    	form.set(IComment.COMMENT_TYPE, comment.get(IComment.COMMENT_TYPE));
	    	form.set(IComment.ID_VALUE, comment.get(IComment.ID_VALUE));
	    	form.set(IComment.PARENT_ID, comment.get(IComment.PK_ID));
	...
	private void initReplyLayout(){
		new ListService().listBeans(ModelNames.COMMENT, IComment.PARENT_ID, getCurState().getPkId(), new ListService.Listener(){
			@Override
	        public void onFailure(Throwable caught) {
