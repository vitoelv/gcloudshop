...
//				});
    	...
    	select = new ColumnConfig(IShipping.PK_ID, "", 50);
		columns.add(select);
    	ColumnConfig colShippingName = new ColumnConfig(IShipping.SHIPPING_NAME, Resources.constants.OrderShipping_shippingName(), 100);
		columns.add(colShippingName);
		ColumnConfig colShippingDesc = new ColumnConfig(IShipping.SHIPPING_DESC, Resources.constants.OrderShipping_shippingDesc(), 100);
    	...
    	grid.setLoadMask(true);
     	...
     	grid.setAutoExpandColumn(IShipping.SHIPPING_DESC);
	...
	private void updateOrder() {
		...
		new ReadService().getBean(ModelNames.ORDERINFO, orderId, new ReadService.Listener() {
			public void onSuccess(final BeanObject bean) {
				new RemoteService().getSpecialService().getOrderFee(getCurState().getPkId(), shippingId, null, new AsyncCallback<Map<String, Object>>() {
