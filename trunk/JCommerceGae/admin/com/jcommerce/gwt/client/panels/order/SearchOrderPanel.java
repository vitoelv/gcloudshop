...
public class SearchOrderPanel extends ContentWidget {
	...
	protected void search() {				
		...
		if(buyer != null) {
			...
			criteria.addCondition(new Condition(IUser.USER_NAME, Condition.EQUALS, buyer));
					...
					new ListService.Listener() {
						public synchronized void onSuccess(List<BeanObject> result) {
							...
							for (Iterator<BeanObject> it = result.iterator(); it
								...
								userId = user.getString(IUser.PK_ID);
							}							
...
//			condition.add(IOrderInfo.DISTRICT + ":" + districtList.getValue(districtList.getSelectedIndex()));
				...
				new ListService.Listener() {
					public synchronized void onSuccess(List<BeanObject> result) {
						for (Iterator<BeanObject> it = result.iterator(); it
							...
							payMannerList.addItem(payment.getString(IPayment.PAY_NAME),
									payment.getString(IPayment.PK_ID));
						}
					}
				});
				...
				new ListService.Listener() {
					public synchronized void onSuccess(List<BeanObject> result) {
						for (Iterator<BeanObject> it = result.iterator(); it
							...
							shipMannerList.addItem(shipping.getString(IShipping.SHIPPING_NAME),
									shipping.getString(IShipping.PK_ID));
						}
					}
				});
	...
	private void populateRegionList(final ListBox regionList, String regionType, String parentId) {
		Criteria criteria = new Criteria();
		criteria.addCondition(new Condition(IRegion.REGION_TYPE, Condition.EQUALS, regionType));
		if(parentId != null) {
			criteria.addCondition(new Condition(IRegion.PARENT_ID, Condition.EQUALS, parentId));
		}
		regionList.clear();
				...
				new ListService.Listener() {
					public synchronized void onSuccess(List<BeanObject> result) {
						for (Iterator<BeanObject> it = result.iterator(); it
							...
							regionList.addItem(province.getString(IRegion.REGION_NAME),
									province.getString(IRegion.PK_ID));
						}
					}
				});		
	}
