...
public class CommentListPanel extends ContentWidget {
	...
	protected void onRender(Element parent, int index) {
	    ...
	    store.addStoreListener(new StoreListener<BeanObject>() {
	    	public void storeDataChanged(StoreEvent<BeanObject> se) {
				...
				for (BeanObject object : storeData) {
					String userId = object.get(IComment.USER_ID);
					if ( userId == null ){
						object.set(IComment.USER_NAME, Resources.constants.CommentList_anonymous());
					}
					
					Long commentType = object.get(IComment.COMMENT_TYPE);
					if ( commentType.equals(IComment.TYPE_GOODS) ) {
						object.set(IComment.COMMENT_TYPE, Resources.constants.CommentList_goods());
						
					}
					else if ( commentType.equals(IComment.TYPE_ARTICLE )) {
						object.set(IComment.COMMENT_TYPE, Resources.constants.CommentList_article());
						
					}
					
					Long status = object.get(IComment.STATUS);
					if ( status.equals(IComment.STATUS_ACTIVE )){
						object.set(IComment.STATUS, Resources.constants.CommentList_active());
					}
					else if ( status.equals( IComment.STATUS_INACTIVE )){
      ...
      act.setAction("viewComment($pkId)");
      render.addAction(act);
      act = new ActionCellRenderer.ActionInfo();
      act.setImage(GWT.getModuleBaseURL()+"icon_trash.gif");
		act.setAction("deleteComment($pkId)");
	...
	private void executeAction(final List<BeanObject> items, String action) {
		...
		for (BeanObject item : items) {
			if ("deleteComment".equals(action)) {
				...
				deleteComment(item.getString(IComment.PK_ID), listener);
			} else if ("statusActive".equals(action)) {
				if (!(IComment.STATUS_ACTIVE).equals(item.get(IComment.STATUS))) {
				...
				if (!(IComment.STATUS_INACTIVE).equals(item.get(IComment.STATUS))) {
	...
	private void updateComment(BeanObject comment, UpdateService.Listener listener) {
		new UpdateService().updateBean(comment.getString(IComment.PK_ID), comment,
				listener);
	}
