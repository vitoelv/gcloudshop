...
public class OrderGoodsPanel extends ContentWidget{
	...
	private void renderGoodsPanel() {
        ...
        act.setAction("deleteOrderGoods($pkId)");
	...
	private void selectGoods() {
		...
		new ReadService().getBean(ModelNames.GOODS, goodsId, new ReadService.Listener() {
			public void onSuccess(BeanObject bean) {
				goods = bean;
				goodsNameLabel.setText((String) bean.get(IGoods.GOODS_NAME));
				goodsSnLabel.setText(String.valueOf(bean.get(IGoods.GOODS_SN)));
				String categoryId = bean.get(IGoods.CAT_ID);
				String branId = bean.get(IGoods.BRAND_ID);
				
				new ReadService().getBean(ModelNames.BRAND, branId, new ReadService.Listener() {
					public void onSuccess(BeanObject bean) {
						brandLabel.setText((String) bean.get(IBrand.BRAND_NAME));
					}
				});
				
				new ReadService().getBean(ModelNames.CATEGORY, categoryId, new ReadService.Listener() {
					public void onSuccess(BeanObject bean) {
						categoryLabel.setText((String) bean.get(ICategory.CAT_NAME));
					}
				});
...
//							}
		...
		new ListService().listBeans(ModelNames.GOODS, new ListService.Listener() {
			public void onSuccess(List<BeanObject> beans) {
				for(BeanObject bean : beans) {
					String goodsName = bean.get(IGoods.GOODS_NAME);
					String goodsSN = bean.get(IGoods.GOODS_SN);
					lbGoods.addItem((String)bean.get(IGoods.GOODS_NAME) + " " + bean.get(IGoods.GOODS_SN), (String)bean.get(IGoods.PK_ID));
				}
			}
		});
	...
	private void addGoodsToOrder() {
		if(lbGoods.getSelectedIndex() == 0) {
			...
			props.put(IOrderGoods.GOODS_ID, lbGoods.getValue(lbGoods.getSelectedIndex()));
			props.put(IOrderGoods.ORDER_ID, getCurState().getPkId());
			props.put(IOrderGoods.GOODS_NAME, goods.get(IGoods.GOODS_NAME));
			props.put(IOrderGoods.GOODS_NUMBER, goodsNum);
			props.put(IOrderGoods.GOODS_PRICE, goodsPrice);
			props.put(IOrderGoods.GOODS_SN, goods.get(IGoods.GOODS_SN));
			props.put(IOrderGoods.MARKET_PRICE, goods.get(IGoods.MARKET_PRICE));
