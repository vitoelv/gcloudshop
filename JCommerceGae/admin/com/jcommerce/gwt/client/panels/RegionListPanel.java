			...
			public void handleEvent(ComponentEvent be) {
				...
				if( value !=null && value.length() > 0 ){
					if(!value.equals(oldValue)){
						region.set(IRegion.REGION_NAME, value);
...
//		fParentId.setValue(parentId ==null? "" : parentId);
		...
		if (parentId != null) {
			...
			Condition cond = new Condition(IRegion.PARENT_ID, Condition.EQUALS,
					parentId);
			...
			new ReadService().getBean(ModelNames.REGION, parentId,
					new ReadService.Listener() {
						...
						public void onSuccess(BeanObject bean) {
							parentParentId = bean.getString(IRegion.PARENT_ID);
							parentName = bean.getString(IRegion.REGION_NAME);
							String parentRegionType = bean.getString(IRegion.REGION_TYPE);
							currentRegionType = String.valueOf(Long.valueOf(parentRegionType)+1);
			...
			parentParentId = null;
			parentName = null;
			currentRegionType = IRegion.TYPE_COUNTRY.toString();
		...
		Criteria criteria = new Criteria();
		Condition cond = new Condition(IRegion.PARENT_ID, Condition.EQUALS, parentId);
		criteria.addCondition(cond);
	...
	private void onAddClicked() {
		try {
			...
			Map<String, Object> props = FormUtils.getPropsFromForm(addPanel);
			props.put(IRegion.PARENT_ID, getCurState().getParentId());
			props.put(IRegion.REGION_TYPE, currentRegionType);
			final BeanObject form = new BeanObject(ModelNames.REGION, props);
